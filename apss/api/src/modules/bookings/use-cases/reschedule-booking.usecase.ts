...
import { bookingRescheduledEvent } from 'src/model/domain/events/booking-rescheduled.event';
...
export class RescheduleBookingUseCase {
    ...
    async exec(input: {
        ...
        const finalStaffId = (input.staffId ?? booking.staffId) ?? undefined;
        ...
        const ok = avail.slots.some(
            (s) => s.startAt === input.startAt.toISOString() && s.endAt === endAt.toISOString(),
        );
        ...
        const updated = await this.bookingsRepo.updateTimes({
            businessId: input.businessId,
            ...
            staffId: finalStaffId ?? null, // <- si tu port no acepta, quítalo
        } as any);
